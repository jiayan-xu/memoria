...
fn near_dup_survives_restart() {
    ...
    // 确认向量已落持久表（重启可用）
    assert!(memoria_core::vector::persist::get_stored_vector(&engine1.pool, &id_a).is_some(),
    ...
    assert!(engine2.hnsw.len() >= 1, "重启后 HNSW 应已从 memory_vectors 重建出向量");
    ...
    assert_eq!(r2.action, "superseded_near_dup", "重启后近义句必须进入同一 supersede 链");
    assert_eq!(r2.superseded_ids, vec![id_a.clone()], "B 的 superseded 目标必须是 A");
    assert_eq!(superseded_by(&engine2.pool, &id_a), Some(r2.id.clone()), "memories.superseded_by 必须等于 B（记在旧记忆 A 上）");
}
...
fn near_dup_below_threshold_not_merged() {
    ...
    assert_eq!(superseded_by(&engine.pool, &r2.id), None, "B.superseded_by 必须为 NULL");
...
fn near_dup_respects_namespace() {
    ...
    assert_eq!(r2.action, "created", "跨 ns 的近义句不应被其它 ns 的记忆 supersede");
    assert!(r2.superseded_ids.is_empty(), "跨 ns 不得合并");
    assert_eq!(superseded_by(&engine.pool, &r2.id), None, "B.superseded_by 必须为 NULL（NS 隔离）");
    assert_eq!(superseded_by(&engine.pool, &r1.id), None, "A 不受影响");
}
