...
fn pattern_conflict_uses_supersede_not_delete() {
    ...
    let inc =
        hybrid_search(&engine.pool, "周末", &ns, 10, None, None, None, None, true).expect("all");
    assert!(
...
fn enrich_ledger_join_matches_context() {
    ...
    .expect("remember");
    seed_entity_mention(
        &engine, &ns, "ent:mem", "Memoria", "system", &mem.id, "ledger",
    );
    ...
    let row = ledger
        ...
        .expect("row");
    assert_eq!(row["entities"].as_array().map(|a| a.len()).unwrap_or(0), 1);
}
