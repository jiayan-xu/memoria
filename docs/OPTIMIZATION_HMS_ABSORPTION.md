...
- ~~retain/consolidate 时用 LLM 抽取信号并持久化~~ → **P2.2d 已做（agent-core consolidate + 可选 retain）**
...
- ~~tags 持久化信号（P2.2 未做）~~ → **P2.2c 已做（remember 写入 `signal:*` tags）**

### P2.2 验收（c/d）

| ID | 验收标准 |
|----|----------|
| **P2.2c** | `memory_remember` 写入时把确定性 text_signals 落到 tags（`signal:num:` / `signal:date:` / `signal:update:`）；ledger 读时合并；`MEMORIA_TEXT_SIGNALS_PERSIST=0` 可关写入 |
| **P2.2d** | agent-core `consolidate` 写 pattern 前 LLM 抽取 signal tags 并随 remember 持久化；可选 `AGENT_TEXT_SIGNALS_LLM_RETAIN=1` 开启 chat retain 路径 |
...
**P2.2 余项（2026-07-17）：**
- **P2.2c** tags 持久化 `signal:*`（remember 写入 + ledger 读时合并；env 可关）
- **P2.2d** agent-core consolidate LLM 抽取 signal tags 持久化（可选 retain 开关）
...
| 2026-07-17 | Phase P2.2c/d：tags signal 持久化 + consolidate LLM 抽取 |
