...
## 2026-07-23 — 图谱详情 Not Found + 部门文档入库

### 图谱点击 Not Found（P0）
- 根因：GET /api/memories/{id} 动态段在 merge+auth layer 下 handler 不执行（空 404）
- 规避：单条 CRUD 改 GET|PUT|DELETE /api/memories?id=；前端 etchMemoryById/保存/删除同步
- 证据：docs/evidence/graph-detail-not-found-2026-07-23.md

### 部门共享文档入库
- HTTP POST/GET /api/documents；MCP ingest_document；Web「文档」页
- 支持 PDF/DOCX/xlsx/xls；默认 ns=org/cs-pufa-2nd-thermal/dept/gufei
- 证据：docs/evidence/dept-document-ingest-2026-07-23.md
