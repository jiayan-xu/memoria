...
pub struct BackupResult {
  ...
  std::fs::create_dir_all(&tier_dir)
    .map_err(|e| format!("mkdir backup dir: {}", e))?;
  ...
  if let Some(vec_path) = vector_index_path {
    ...
    if vec_src.exists() {
      let vec_dst = tier_dir.join(format!(
        "hnsw_vectors_{}.bin",
        timestamp
      ));
...
fn check_disk_space(path: &Path) -> Result<(), String> {
  ...
  #[cfg(not(target_os = "windows"))]
  {
    ...
    let mut buf = Statvfs {
      ...
      files: 0, ffree: 0, favail: 0, fsid: 0, flag: 0, namemax: 0, _pad: [0; 48],
    };
