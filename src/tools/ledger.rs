//! Ledger enrichment — HMS 类型化证据账本（Phase A / O1–O6）。
//!
//! **仅**由 `memory_context` 调用（O6）。每行：
//! `type` / `occurred`（优先 tags `occurred:YYYY-MM-DD`）/ `mentioned`（valid_from）/
//! `source_ref` / `entities`（P0 硬 `[]`，O1）/ score。
//! `event_time` 列仅作只读兼容兜底，不以之为 P0 写入主路径（O2）。
    ...
    fused
        ...
        .map(|(i, f)| {
            ...
            json!({
                ...
                // O1: P0 硬空，不 JOIN
                "entities": serde_json::Value::Array(vec![]),
                "is_latest": true,
            })
        })
        .collect()
