...
pub fn remember_with_dedup(
    ...
    if let (Some(hnsw_idx), Some(qc)) = (hnsw, query_cache) {
        // 从 query cache 获取 content 的 embedding
        // Python 端在调 memory_remember 之前会先 cache_query_vector(content, vector)
        if let Some(query_vector) = qc.get(content) {
            // 搜索 top-10 结果（排除自身）
            if let Ok(results) = hnsw_idx.search(&query_vector, 10) {
