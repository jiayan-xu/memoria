...
fn check_sqlite_rw(pool: &SqlitePool) -> CheckResult {
  ...
  let result = (|| -> Result<(), String> {
    conn
      .execute_batch("CREATE TABLE IF NOT EXISTS _health_check (id INTEGER);")
      .map_err(|e| format!("create: {}", e))?;
    conn
      .execute("INSERT INTO _health_check (id) VALUES (1)", [])
      .map_err(|e| format!("insert: {}", e))?;
    conn
      .execute("DELETE FROM _health_check WHERE id = 1", [])
      .map_err(|e| format!("delete: {}", e))?;
    conn
      .execute_batch("DROP TABLE IF EXISTS _health_check;")
...
fn check_schema_version(pool: &SqlitePool) -> CheckResult {
  ...
  if user_version >= EXPECTED_SCHEMA_VERSION as i64 {
    CheckResult {
      name: "schema_version".to_string(),
      status: "pass".to_string(),
      message: format!("user_version = {} (expected >= {})", user_version, EXPECTED_SCHEMA_VERSION),
      duration_ms,
    }
  } else {
    // 尝试升级 schema 版本
...
fn check_fts5(pool: &SqlitePool) -> CheckResult {
  ...
  let result: Result<i64, _> = conn.query_row(
    "SELECT COUNT(*) FROM memories_fts LIMIT 1",
    [],
    |r| r.get(0),
  );
  let duration_ms = start.elapsed().as_millis() as u64;
